...
express their desire to receive each one (1–5 stars), and eventually record
the final attribution of each item.
...
- Define the **column mapping** for the sheet: the app must not hard-code
  ...
  logical field the app understands (matching is case/whitespace-insensitive):
  - `itemNumber` — sequential item number (auto-generated by the app when
    adding an item: highest existing number in that tab + 1; not
    user-edited)
  ...
  - one or more **desire columns** — a configurable list of person names,
    ...
    (default: `Fanny`, `Antoine`, `Flo`, `Marion`)
...
- The default structure to support out of the box (used to pre-fill
  ...
  | N° | Nom | Photo | Fanny | Antoine | Flo | Marion | Validation |
  |---|---|---|---|---|---|---|---|
...
- Fill in the remaining fields (designation, desires) and pick the target
  tab/category. The item number is computed automatically, not entered.
